<#
.SYNOPSIS 
    This sample automation function timer trigger can call the GetVMToStart function. You can configure
...
$FunctionName = "GetVMToStart"

# Modify these for specific requirements to find machines to start
$JsonBody = @'
{
...
if ($null -ne $Env:MSI_ENDPOINT)
{
    ...
    $GetVMToStart = "https://$env:APPSETTING_WEBSITE_SITE_NAME.azurewebsites.net/api/$($FunctionName)?code=$($FunctionCode.keys.value)"
    
    # Start all VMs in the resource group by calling the function with the body of VMs to look for
    Invoke-RestMethod $GetVMToStart -Method Post -Body $JsonBody
